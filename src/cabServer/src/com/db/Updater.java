...
public class Updater implements Runnable{
    ...
    public Updater(MainFrame ui, Map map){
        this.ui = ui; this.map = map; df = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss"); df.setLenient(true);
    }
                    ...
                    for(Element e : map.obj){
                        if(e.type == 2){
                            try{
                                ...
                                while(rs.next()){
                                    Date date = df.parse(rs.getString("time")); System.out.println("Hey");   
                                    ...
                                    if(compareDates(today, date) > 0){ ui.db.execute("DELETE FROM cab_"+e.id+" WHERE seq="+seq); }
                                        ...
                                        if(today.getHours() == date.getHours() && today.getMinutes() == date.getMinutes() && rs.getString("temp").equals("#")){
                                            ...
                                            ui.db.execute("UPDATE cab_"+e.id+" SET temp='#' WHERE seq="+seq);
